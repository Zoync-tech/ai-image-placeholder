-- Safe Supabase Database Schema for AI Image Placeholder
...
DROP POLICY IF EXISTS "Allow all operations on email verifications" ON email_verifications;
DROP POLICY IF EXISTS "Users can view own image generations" ON image_generations;
DROP POLICY IF EXISTS "Users can insert own image generations" ON image_generations;
...
-- Functions for credit management (safe to run multiple times)
...
-- Add updated_at triggers (safe to run multiple times)

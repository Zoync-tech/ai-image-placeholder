-- ULTRA NUCLEAR OPTION: Find and destroy ALL conflicting functions
-- This will be more aggressive in finding and removing duplicates

-- Step 1: Show all existing functions (for debugging)
SELECT 
  routine_schema,
  routine_name, 
  routine_type,
...
-- Step 2: Drop ALL functions more aggressively
DO $$ 
DECLARE
    func_record RECORD;
BEGIN
    ...
    FOR func_record IN 
        SELECT routine_name, routine_type, specific_name, routine_schema
        FROM information_schema.routines 
        WHERE routine_schema IN ('public', 'auth')
        AND (
            routine_name LIKE '%prompt_hash%' OR
    ...
    LOOP
        EXECUTE 'DROP ' || func_record.routine_type || ' IF EXISTS ' || 
                func_record.routine_schema || '.' || func_record.specific_name || ' CASCADE';
        RAISE NOTICE 'Dropped: %', func_record.routine_schema || '.' || func_record.specific_name;
    END LOOP;
END $$;
...
-- Step 8: Create ONE SINGLE function with explicit schema
...
BEGIN
  prompt_hash_value := public.generate_prompt_hash(prompt_text, dimensions_text, format_text);
...
-- Step 9: Verify only one function exists
SELECT 
  routine_name, 
  routine_type,
  specific_name
...
AND routine_name = 'generate_prompt_hash'
ORDER BY routine_name;

SELECT 'Ultra nuclear cleanup completed! Only one function should exist now.' as message;
